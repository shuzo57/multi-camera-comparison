...
objpoints = np.array(
    [cube_3d for _ in range(len(imgpoints))], dtype=np.float32
)
...
for data_num in range(10):
    ...
    cam1_position = pd.read_csv(
        os.path.join(cam1_dir, FileName.position_data), index_col="frame"
    )
    cam2_position = pd.read_csv(
        os.path.join(cam2_dir, FileName.position_data), index_col="frame"
    )
    cam3_position = pd.read_csv(
        os.path.join(cam3_dir, FileName.position_data), index_col="frame"
    )
    cam4_position = pd.read_csv(
        os.path.join(cam4_dir, FileName.position_data), index_col="frame"
    )
    cam5_position = pd.read_csv(
        os.path.join(cam5_dir, FileName.position_data), index_col="frame"
    )
    ...
    min_frame = max(
        cam1_position.index.min(),
        cam2_position.index.min(),
        cam3_position.index.min(),
        cam4_position.index.min(),
        cam5_position.index.min(),
    )
    max_frame = min(
        cam1_position.index.max(),
        cam2_position.index.max(),
        cam3_position.index.max(),
        cam4_position.index.max(),
        cam5_position.index.max(),
    )
    ...
    for n in range(5):
        for combo_indices in combinations(range(5), n + 2):
            ...
            for frame in range(int(min_frame), int(max_frame) + 1):
                poses = np.array(
                    [
                        cam_positions[i].loc[frame].values.reshape(-1, 2)
            ...
            position_df.to_csv(
                f"{output_dir}/hirasaki_{data_num}_{''.join([str(i) for i in combo_indices])}.csv"
            )

            if cam == 5:
                plot_3d_motion_exp(
                    position_df / 1000,
                    output_name=f"html/hirasaki_{data_num}.html",
                )
